...
class StockRestoreRequestedOutboxAdapterTest {

    @Test
    @DisplayName("재고 복구 요청 이벤트를 outbox 이벤트로 저장한다")
    fun saveStockRestoreRequestedEvent() {
        // given
        val event = StockRestoreRequestedEvent(
            ...
            bookId = 10L,
            quantity = 1L,
            occurredAt = LocalDateTime.of(2026, 6, 14, 12, 0),
        )
        ...
        val adapter = StockRestoreRequestedOutboxAdapter(
        ...
        assertEquals(StockRestoreRequestedEvent.EVENT_TYPE, outBoxRecord.captured.eventType)
        assertEquals(StockRestoreRequestedEvent.TOPIC, outBoxRecord.captured.topic)
